...
BUILD_VERSION = "ETH_CFBENCHMARKS_ETHUSD_RTI_5SLOT_STREAK_EXITS_DUPGUARD_V1_2026-08-20"
...
SERIES = os.getenv("SERIES", "KXETH15M")
...
DEBUG_LIVE_DATA = os.getenv("DEBUG_LIVE_DATA", "false").strip().lower() in {"1", "true", "yes", "on"}
...
CFBENCHMARKS_STREAM_ENABLED = os.getenv("CFBENCHMARKS_STREAM_ENABLED", "true").strip().lower() in {"1", "true", "yes", "on"}
CFBENCHMARKS_INDEX_ID = os.getenv("CFBENCHMARKS_INDEX_ID", "ETHUSD_RTI").strip() or "ETHUSD_RTI"
...
STRATEGY_SLOTS = [
    {
        ...
        "max_entry_cents": int(os.getenv("STREAK_A_MAX_ENTRY_CENTS", "17")),
        "entry_window_minutes": float(os.getenv("STREAK_A_ENTRY_WINDOW_MINUTES", "2")),
        "contracts": float(os.getenv("STREAK_A_CONTRACTS", "1")),
        "stop_loss_cents": float(os.getenv("STREAK_A_STOP_LOSS_CENTS", "0")),
        "sell_early_cents": float(os.getenv("STREAK_A_SELL_EARLY_CENTS", "100")),
    },
    {
        "name": "B",
        "enabled": env_bool("STREAK_B_ENABLED", True),
        "streak": int(os.getenv("STREAK_B_LENGTH", "3")),
        "max_entry_cents": int(os.getenv("STREAK_B_MAX_ENTRY_CENTS", "21")),
        "entry_window_minutes": float(os.getenv("STREAK_B_ENTRY_WINDOW_MINUTES", "1")),
        "contracts": float(os.getenv("STREAK_B_CONTRACTS", "1")),
        "stop_loss_cents": float(os.getenv("STREAK_B_STOP_LOSS_CENTS", "0")),
        "sell_early_cents": float(os.getenv("STREAK_B_SELL_EARLY_CENTS", "100")),
    },
    {
        "name": "C",
        "enabled": env_bool("STREAK_C_ENABLED", True),
        "streak": int(os.getenv("STREAK_C_LENGTH", "4")),
        "max_entry_cents": int(os.getenv("STREAK_C_MAX_ENTRY_CENTS", "38")),
        "entry_window_minutes": float(os.getenv("STREAK_C_ENTRY_WINDOW_MINUTES", "1")),
        "contracts": float(os.getenv("STREAK_C_CONTRACTS", "1")),
        "stop_loss_cents": float(os.getenv("STREAK_C_STOP_LOSS_CENTS", "0")),
        "sell_early_cents": float(os.getenv("STREAK_C_SELL_EARLY_CENTS", "100")),
    },
    {
        "name": "D",
        "enabled": env_bool("STREAK_D_ENABLED", True),
        "streak": int(os.getenv("STREAK_D_LENGTH", "5")),
        "max_entry_cents": int(os.getenv("STREAK_D_MAX_ENTRY_CENTS", "16")),
        "entry_window_minutes": float(os.getenv("STREAK_D_ENTRY_WINDOW_MINUTES", "5")),
        "contracts": float(os.getenv("STREAK_D_CONTRACTS", "1")),
        "stop_loss_cents": float(os.getenv("STREAK_D_STOP_LOSS_CENTS", "0")),
        "sell_early_cents": float(os.getenv("STREAK_D_SELL_EARLY_CENTS", "100")),
    },
    {
        "name": "E",
        "enabled": env_bool("STREAK_E_ENABLED", False),
        "streak": int(os.getenv("STREAK_E_LENGTH", "6")),
        "max_entry_cents": int(os.getenv("STREAK_E_MAX_ENTRY_CENTS", "26")),
        "entry_window_minutes": float(os.getenv("STREAK_E_ENTRY_WINDOW_MINUTES", "7")),
        "contracts": float(os.getenv("STREAK_E_CONTRACTS", "1")),
        "stop_loss_cents": float(os.getenv("STREAK_E_STOP_LOSS_CENTS", "0")),
        "sell_early_cents": float(os.getenv("STREAK_E_SELL_EARLY_CENTS", "100")),
    },
]
...
for slot in STRATEGY_SLOTS:
    ...
    if not 0 <= slot["stop_loss_cents"] <= 99:
        raise RuntimeError(
            f'STREAK_{slot["name"]}_STOP_LOSS_CENTS must be between 0 and 99; use 0 to disable'
        )
    if not 1 <= slot["sell_early_cents"] <= 100:
        raise RuntimeError(
            f'STREAK_{slot["name"]}_SELL_EARLY_CENTS must be between 1 and 100; use 100 to hold'
        )
...
SESSION_LOG = os.path.join(DATA_DIR, "eth_session_results.csv")
...
def extract_kalshi_start_eth(market):
    """
...
def extract_eth_price_from_live_data(live_data, start_eth):
    """
    Extract the ETH price from Kalshi's own live-data payload.
    ...
    and be near the session's displayed ETH target. Target/strike fields are
    ...
    preferred = ("eth", "underlying", "index", "spot", "current", "last", "price", "value")
    excluded = (
    ...
    for path, number in recursive_numeric_candidates(details):
        ...
        if start_eth is not None:
            if number < start_eth * Decimal("0.5") or number > start_eth * Decimal("1.5"):
                continue
        elif number < Decimal("100"):
            continue

        score = 0
        weights = {
            "eth": 12, "underlying": 10, "index": 9, "spot": 8,
...
def fetch_live_data_for_milestone(milestone, start_eth, context="boundary"):
    """
    ...
    No external ETH source is ever used.
    """
    ...
    try:
        ...
        price, path = extract_eth_price_from_live_data(live_data, start_eth)
        if price is not None:
            return price, "kalshi_live_data", path, milestone_id
        print(f"LIVE DATA CURRENT NO ETH CANDIDATE: id={milestone_id} payload={_short_json(live_data)}")
    ...
    if milestone_type:
        try:
            ...
            print(
                f"LIVE DATA LEGACY NO ETH CANDIDATE: id={milestone_id} "
...
def fetch_kalshi_displayed_eth(market, start_eth):
    """
    Kalshi-only live ETH display.

    We intentionally do not call Coinbase, Binance, TradingView, or any other
    external ETH source. If Kalshi does not expose a usable live ETH value for
    ...
    for milestone in milestones:
        price, source, path, milestone_id = fetch_live_data_for_milestone(
            milestone, start_eth, context="post_close_lookup"
        )
        if price is not None:
...
def choose_current_open_session():
    """
    Pick the currently active KXETH15M session by its scheduled open/close
...
def prepare_current_session():
    ...
    Cache everything we can before the boundary:
      - exact ticker
      - Kalshi-displayed start/target ETH price
      - close timestamp
    ...
    start_eth, start_source = extract_kalshi_start_eth(market)
    ...
    print(
        ...
        f"start={format_decimal(start_eth) or '?'} "
        f"close_time={market.get('close_time')} "
...
def fetch_prepared_kalshi_close(prepared):
    ...
    documented Kalshi live-data endpoints; no external ETH source is used.
    """
    start_eth = prepared.get("start_eth")
    ...
    for milestone in milestones:
        ...
        price, source, path, milestone_id = fetch_live_data_for_milestone(
            milestone, start_eth, context="exact_boundary"
        )
        if price is not None:
            print(
...
def diagnostic_probe_prepared_session(prepared):
    ...
    """
    Probe Kalshi live-data for the currently prepared ETH session during the
    ...
    for milestone in milestones:
        price, source, path, milestone_id = fetch_live_data_for_milestone(
            milestone, start_eth, context="one_minute_diagnostic"
        )
        if price is not None:
            found = {
                "price": price,
            ...
            print(
                f"LIVE ETH FOUND: id={milestone_id} "
                f"value={format_decimal(price)} source={source} path={path}"
            )
            break

    if found is None:
        print("LIVE TEST RESULT: no usable Kalshi ETH live value found", flush=True)
...
def capture_prepared_session(prepared):
    ...
    if close_time is None:
        return append_session_result(
            market, prepared.get("start_eth"), None, "", "", "", 9999,
    ...
    if start_eth is None:
        return append_session_result(
            market, None, None, "", "", "", capture_lag,
            note="Kalshi displayed starting/target ETH price unavailable before close"
        )

    close_eth, source, live_path, milestone_id = fetch_prepared_kalshi_close(prepared)

    if close_eth is None:
        return append_session_result(
            market,
            start_eth,
            None,
            ...
            capture_lag,
            note=(
                "Kalshi live ETH display unavailable from cached live_data "
...
def get_latest_market_closed_by_time():
    """
    Find the newest KXETH15M session whose scheduled close_time has passed.
...
def append_session_result(market, start_eth, close_eth, source, live_path, milestone_id,
                          capture_lag, note=""):
    ...
    if start_eth is not None and close_eth is not None:
        close_distance = close_eth - start_eth
        if close_distance > 0:
    ...
    rows.append({
        ...
        "kalshi_start_eth": format_decimal(start_eth),
        "kalshi_close_eth": format_decimal(close_eth),
    ...
    if immediate_result in {"yes", "no", "tie"}:
        print(
            f"IMMEDIATE CLOSE: {ticker} start={format_decimal(start_eth)} "
            f"close={format_decimal(close_eth)} distance={format_decimal(close_distance)} "
...
def capture_just_closed_session():
    ...
    # Never use a current ETH display value as though it were the historical
    # close if the bot woke up too late (e.g. after a restart).
    if capture_lag > MAX_CLOSE_CAPTURE_LAG_SECONDS:
        return append_session_result(
            market, *extract_kalshi_start_eth(market)[:1], None,
            "", "", "", capture_lag,
            note=(
    ...
    if start_eth is None:
        return append_session_result(
            market, None, None, "", "", "", capture_lag,
            note="Kalshi displayed starting/target ETH price unavailable"
        )

    close_eth, close_source, live_path, milestone_id = fetch_kalshi_displayed_eth(
        market, start_eth
    )
    if close_eth is None:
        return append_session_result(
            market, start_eth, None, "", "", milestone_id, capture_lag,
            note=(
                "Kalshi live ETH display unavailable from live_data; "
...
def session_rows_as_markets():
    ...
    for row in rows:
        ...
        markets.append({
            ...
            "result": result,
            "kalshi_start_eth": row.get("kalshi_start_eth"),
            "kalshi_close_eth": row.get("kalshi_close_eth"),
...
def print_last_five_immediate(markets):
    ...
    for m in recent:
        print(
            ...
            f"start={m.get('kalshi_start_eth','')} "
            f"close={m.get('kalshi_close_eth','')} "
...
def get_recent_kxeth15m_markets_for_strikes():
    """
...
def eth_session_start(market):
    """
    Actual KXETH15M ETH measurement interval starts 15 minutes before close_time.
    Kalshi's API open_time can be earlier because a market may be listed before
    its 15-minute ETH interval begins.
    """
...
def market_strike_record(market):
    strike, source = extract_kalshi_start_eth(market)
    if strike is None:
        return None
    return {
        "ticker": market.get("ticker"),
...
def derive_successive_strike_results():
    ...
    No official Kalshi result, Past page, settlement average, or external ETH
    source is used.
    """
    markets = get_recent_kxeth15m_markets_for_strikes()
    strikes = []
    now = utc_now()
    ...
    for market in markets:
        ...
        # 15-minute ETH session has begun: close_time - 15 minutes.
        session_start = eth_session_start(market)
    ...
    for prior, nxt in zip(strikes, strikes[1:]):
        ...
        derived.append({
            ...
            "kalshi_start_eth": prior_strike,
            "kalshi_close_eth": next_strike,
...
def persist_successive_strike_results(derived):
    """
    Persist newly inferred session results to eth_session_results.csv.
    ...
    for d in derived:
        ...
        if prior_row and (prior_row.get("close_price_source") or "").strip() == "kalshi_cfbenchmarks_ethusd_rti_15m_final":
            # Preserve the exact ETHUSD_RTI boundary capture. Successive strike remains
        ...
        row = {
            ...
            "kalshi_start_eth": format_decimal(d.get("kalshi_start_eth")),
            "kalshi_close_eth": format_decimal(d.get("kalshi_close_eth")),
...
def print_last_five_successive(markets):
    ...
    for m in recent:
        print(
            ...
            f"{m.get('next_ticker','?')} | "
            f"start={m.get('kalshi_start_eth','')} "
            f"next_strike={m.get('kalshi_close_eth','')} "
            f"distance={m.get('close_distance','')} | "
            f"{(m.get('result') or '?').upper()}"
        )
...
def market_age_minutes(market):
    """
    Minutes since the actual ETH 15-minute session began.
...
def market_for_session_start(open_markets, session_start):
    """Return the KXETH15M market whose true session start matches exactly."""
    for m in open_markets:
        start = eth_session_start(m)
...
def _strike_diag_market_snapshot(target_start):
    """Return the exact KXETH15M market for target_start from unopened/open catalogs."""
    found = []
    errors = []
    for status in ("unopened", "open"):
        try:
            ...
            for market in data.get("markets", []):
                start = eth_session_start(market)
    ...
    while True:
        try:
            ...
            if market is None:
                ...
                if now.timestamp() - last_heartbeat >= 10:
                    strike_hb, source_hb = extract_kalshi_start_eth(market)
                    print(
                        f"STRIKE WATCH HEARTBEAT: target_start={target.isoformat()} "
                ...
                strike, strike_source = extract_kalshi_start_eth(market)
                raw_fields = {
...
FAST_LIVE_SOURCES = {
    ...
    "kalshi_cfbenchmarks_ethusd_rti_15m_final",
}
...
def fast_streak_view(immediate_markets, target_start):
    ...
    synthetic = {
        ...
        "kalshi_start_eth": fast_row.get("kalshi_start_eth") or "",
        "kalshi_close_eth": fast_row.get("kalshi_close_eth") or "",
    }
        ...
        try:
            ...
            if market:
                ...
                if bid is not None:
                    ...
                    slot_cfg = next(
                        (
                            s for s in STRATEGY_SLOTS
                            if s["name"] == str(tracker.get("slot", "")).upper()
                        ),
                        None,
                    )
                    stop_setting = (
                        float(slot_cfg.get("stop_loss_cents", 0))
                        if slot_cfg else 0.0
                    )
                    take_setting = (
                        float(slot_cfg.get("sell_early_cents", 100))
                        if slot_cfg else 100.0
                    )
                    ...
                    append_exit_csv(
                        ...
                        POSITION_PRICE_FIELDS,
                        {
                            ...
                            "stop_loss_setting_cents": stop_setting,
                            "sell_early_setting_cents": take_setting,
                        },
                    )
...
def strategy_exit_settings(position):
    """
    Return the streak-slot-specific exit settings for this filled position.

    Each slot owns its stop-loss and early-sell settings:
      stop_loss_cents = 0 disables stop loss
      sell_early_cents = 100 means hold to settlement

    There are intentionally no universal stop-loss, early-sell, or time-band
    exit rules in the ETH build.
    """
    slot_name = str(position.get("slot") or "").strip().upper()
    try:
    ...
    slot = next(
        (s for s in STRATEGY_SLOTS if s["name"] == slot_name),
        None,
    )
    if slot is None and streak:
        slot = next(
            (s for s in STRATEGY_SLOTS if int(s["streak"]) == streak),
            None,
        )

    if slot is None:
        return 0.0, 100.0, f"S{streak}_hold"

    stop_threshold = float(slot.get("stop_loss_cents", 0.0))
    early_threshold = float(slot.get("sell_early_cents", 100.0))

    if not 0 <= stop_threshold <= 99:
        stop_threshold = 0.0
    if not 1 <= early_threshold <= 100:
        early_threshold = 100.0

    if stop_threshold <= 0 and early_threshold >= 100:
        label = f"S{streak}_hold"
    else:
        label = (
            f"S{streak}_stop{stop_threshold:g}_take{early_threshold:g}"
        )
    return stop_threshold, early_threshold, label
...
# -----------------------------------------------------------------------------
# CF BENCHMARKS ETHUSD_RTI WEBSOCKET DIAGNOSTIC -- NO TRADING-RULE CHANGES
# -----------------------------------------------------------------------------
...
async def cfbenchmarks_stream_loop():
    ws_url = cfbenchmarks_ws_url()
    while CFBENCHMARKS_STREAM_ENABLED:
        try:
...
def cfb_final_value_for_boundary(boundary_dt):
    """Return the ETHUSD_RTI quarter-hour final-minute average for exactly boundary_dt."""
    with _CFB_STATE_LOCK:
...
def capture_prepared_from_cfb_if_due(prepared):
    """Use Kalshi's authenticated ETHUSD_RTI final 60-second average as the boundary close.

    Returns (captured, prepared_after). This is the live trading trigger path.
    It never substitutes a stale ETHUSD_RTI tick or the delayed next-session strike.
    """
        ...
        if final_value is not None:
            ...
            appended = append_session_result(
                ...
                note=(
                    ...
                    "cfbenchmarks_index=ETHUSD_RTI; window_size=60; "
...
def cfbenchmarks_thread_worker():
    try:
        asyncio.run(cfbenchmarks_stream_loop())
    except Exception as exc:
        print(f"CFB WS THREAD FATAL: {exc!r}")


def start_cfbenchmarks_stream_thread():
    if not CFBENCHMARKS_STREAM_ENABLED:
        print("CFBENCHMARKS STREAM=OFF")
        return None
    print(
        f"CFBENCHMARKS STREAM=ON index={CFBENCHMARKS_INDEX_ID} "
        f"reconnect={CFBENCHMARKS_RECONNECT_SECONDS:g}s"
    )
    thread = threading.Thread(
        target=cfbenchmarks_thread_worker,
        daemon=True,
        name="cfbenchmarks-ethusd-rti",
    )
    thread.start()
...
def main():
    ...
    ensure_trade_log()
    ensure_session_log()
    start_cfbenchmarks_stream_thread()
    ...
    print("EXIT CONFIG: streak-specific only; 0c stop=OFF, 100c sell=HOLD", flush=True)

    print("KALSHI ETH 15-MINUTE STREAK BOT")
    if MODE == "paper":
    ...
    for slot in STRATEGY_SLOTS:
        ...
        print(
            ...
            f"stop={slot['stop_loss_cents']:g}c "
            f"sell={slot['sell_early_cents']:g}c"
        )
    print("OUTCOME METHOD=Kalshi CF Benchmarks ETHUSD_RTI final 60-second quarter-hour average; successive strikes retained for verification")
    print("FAST BOUNDARY=ON; ETHUSD_RTI window_size=60 is the live trigger; no wait for next formal strike")
    ...
    print("EXTERNAL ETH SOURCES=not used")
    print(
    ...
    while True:
        try:
            ...
            # live ETH value exactly at the boundary.
            if FAST_BOUNDARY_ENABLED:
                ...
                if prepared_fast_session is not None:
                    ...
                    if target is not None and (
                        ...
                        or eth_session_start(preidentified_next_market) != target
                    ):
                        preidentified_next_market = preidentify_market_for_session_start(target)
                        ...
                        if row:
                            latency_mark(
                                ...
                                detail=(
                                    ...
                                    f"start={row.get('kalshi_start_eth','?')} "
                                    f"close={row.get('kalshi_close_eth','?')} "
            ...
            if FAST_BOUNDARY_ENABLED and fast_ready:
                ...
                latency_mark(
                    ...
                    detail=(
                        ...
                        f"start={fast_row.get('kalshi_start_eth','?')} "
                        f"boundary={fast_row.get('kalshi_close_eth','?')}"
                    ),
                )
                print(
            ...
            pending_api_signal = {
                ...
                "ticker_hint": (
                    ...
                    and eth_session_start(preidentified_next_market) == target_start
                    else None
                ),
            }
            latency_mark(
